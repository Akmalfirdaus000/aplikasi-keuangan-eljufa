...
const formatThousandID = (digits) =>
  digits ? digits.replace(/\B(?=(\d{3})+(?!\d))/g, ".") : ""

/* Untuk tampilan tabel: format currency penuh */
...
export default function Index() {
  ...
  // Form: simpan angka nominal sebagai DIGIT-ONLY string, biar mudah diformat
  const [form, setForm] = useState({
    id: null,
    siswa_id: "",
    kategori_id: "",
    deskripsi: "",
    total_tagihan: "",   // digit-only string
    sisa_tagihan: "",    // digit-only string
    status: "Belum Lunas",
  })
  ...
  // Khusus input rupiah: simpan digit-only, tampilkan dengan titik ribuan
  ...
  const handleSubmit = (e) => {
    ...
    const payload = {
      ...form,
      // convert digit-only string ke number
  ...
  const handleEdit = (tagihan) => {
    ...
    setForm({
      ...
      // pastikan jadi digit-only string
      total_tagihan: onlyDigits(tagihan.total_tagihan),
      sisa_tagihan: onlyDigits(tagihan.sisa_tagihan),
  ...
  const filteredSiswa = useMemo(() => {
    ...
    return (siswaData || []).filter((s) => {
      ...
      const lokalMatch = (s.kelas?.lokal || "").toLowerCase().includes(q)
  ...
  const lokalOptions = useMemo(() => {
    const set = new Set((siswas || []).map((s) => s.kelas?.lokal).filter(Boolean))
    return Array.from(set)
  }, [siswas])
            ...
            <Dialog open={openModal} onOpenChange={setOpenModal}>
              ...
              <DialogContent className="max-w-2xl">
                ...
                <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
                  ...
                  {/* INPUT RUPIAH: tampil 1.000, state simpan digit-only */}
                  <div>
                    ...
                    <Input
                      ...
                      inputMode="numeric"
                      name="total_tagihan"
                      value={formatThousandID(form.total_tagihan)}
                      onChange={handleMoneyChange("total_tagihan")}
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      inputMode="numeric"
                      name="sisa_tagihan"
                      value={formatThousandID(form.sisa_tagihan)}
                      onChange={handleMoneyChange("sisa_tagihan")}
                      placeholder="cth: 1.500.000"
                      required
                    />
                  </div>
