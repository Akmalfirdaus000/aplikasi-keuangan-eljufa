...
export default function LaporanPerKategoriIndex() {
    ...
    const [filterKelas, setFilterKelas] = useState(filters.kelas || "all");
    ...
    const kelasOptions = useMemo(() => {
        ...
        const s = (sekolahList || []).find(
            (x) => x.nama_sekolah === filterSekolah
        );
    ...
    const lokalOptions = useMemo(() => {
        ...
        const set = new Set(
            (s?.kelas || []).map((k) => k.lokal).filter(Boolean)
        );
        ...
        () =>
            (rows || []).map((r) => ({
                ...
                kategori:
                    r.kategori ||
                    (r.kategori_nama
                        ? { id: r.kategori_id, nama: r.kategori_nama }
                        : null),
                siswa:
                    r.siswa ||
                    (r.siswa_nama
                        ? { id: r.siswa_id, nama: r.siswa_nama }
                        : null),
                sekolah:
                    r.sekolah ||
                    (r.sekolah_nama
                        ? { id: r.sekolah_id, nama: r.sekolah_nama }
                        : null),
                kelas:
                    r.kelas ||
                    (r.kelas_nama
                        ? { id: r.kelas_id, nama: r.kelas_nama, lokal: r.lokal }
                        : null),
            })),
        [rows]
    );
    ...
    const filtered = useMemo(() => {
        ...
        if (dateFrom)
            data = data.filter(
                (r) => r.tanggal && new Date(r.tanggal) >= new Date(dateFrom)
            );
        if (dateTo) {
            const t = new Date(dateTo);
            const tplus = new Date(t);
            tplus.setDate(t.getDate() + 1);
            data = data.filter((r) => r.tanggal && new Date(r.tanggal) < tplus);
        }
        if (filterSekolah !== "all")
            data = data.filter((r) => r.sekolah?.nama === filterSekolah);
        if (filterKelas !== "all")
            data = data.filter((r) => r.kelas?.nama === filterKelas);
        if (filterLokal !== "all")
            data = data.filter((r) => (r.kelas?.lokal || "") === filterLokal);
        if (filterKategori !== "all" && filterKategori !== "") {
            data = data.filter(
                (r) => String(r.kategori?.id || "") === String(filterKategori)
            );
        }
    ...
    // Urutan kolom pivot = urutan kategori di DB (kategoriList)
    ...
    }, [filtered, COLUMNS, colIndexById, filterKategori]);
            ...
            header={
                <div className="flex flex-col gap-1">
                    ...
                    <p className="text-sm text-muted-foreground">
                        Pivot dinamis berdasarkan daftar kategori di database.
                    </p>
                </div>
            }
        >
                ...
                {isPivot ? (
                    <Card>
                        <CardHeader className="flex items-center justify-between">
                            <CardTitle>
                                Rekap Per Siswa (Semua Kategori)
                            </CardTitle>
                            <div className="text-sm text-muted-foreground">
                                Total siswa tampil: {pivotRows.length} • Total
                                nominal: {formatRupiah(pivotFooter.total)}
                            </div>
                        </CardHeader>
                        <CardContent>
                    ...
                    // DETAIL MODE tetap sama seperti sebelumnya (pakai `filtered`)
                    <Card>
                        <CardHeader className="flex items-center justify-between">
                            <CardTitle>
                            ...
                            <div className="flex gap-2">
                                <div className="text-sm text-muted-foreground">
                                    Total data: {filtered.length} • Total
